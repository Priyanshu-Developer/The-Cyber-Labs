...
export async function POST(request: NextRequest) {
  ...
  const pool = getPool();
  const sql = `SELECT * FROM hospital_users WHERE username='${username}' AND password='${password}'`;
  ...
  response.cookies.set("lab04_session", token, { httpOnly: true, secure: false, sameSite: "lax", path: "/", maxAge: 7200 });
  return response;
}
